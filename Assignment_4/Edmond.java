...
public static List<Integer> findAllZeroCycles(Hashtable<Integer, PriorityQueue<VertexWeighted>> allValuesClone, int lastVertex) {
    System.out.println(allValuesClone);
    List<Integer> visited = new ArrayList<>();
    // for every node, start from root node 
    for (int i = 1; i <= lastVertex; i++) {
        // create a list to keep track of visited vertices
        // create a stack to keep track of vertices in the current path
        ...
        while (!stack.isEmpty()) {
            ...
            // Create a deep copy of the PriorityQueue for the current vertex
            PriorityQueue<VertexWeighted> originalValues = allValuesClone.get(currentVertex);
            PriorityQueue<VertexWeighted> valuesCopy = new PriorityQueue<>(originalValues);
        
            // Use valuesCopy for the rest of the operations inside the loop
            if (valuesCopy == null || valuesCopy.isEmpty()) {
                stack.pop();
                continue;
            }
            VertexWeighted nextVertex = valuesCopy.poll();
            // if the next vertex is null, remove the vertex from the stack
            if (nextVertex == null) {
                stack.pop();
                continue;
            }
            // if the next vertex is the starting vertex and the weight is 0, we have a cycle
            if (nextVertex.getVertex()[0] == i && nextVertex.getWeight() == 0) {
                List<Integer> cycle = new ArrayList<>();
                while (!stack.isEmpty()) {
                    cycle.add(stack.pop());
                }
                Collections.reverse(cycle); // To get the nodes in the order they were visited
                return cycle; // Return the nodes in the cycle
            }
            // if the next vertex has not been visited, add it to the stack and mark it as visited
            if (!visited.contains(nextVertex.getVertex()[0])) {
                stack.push(nextVertex.getVertex()[0]);
                visited.add(nextVertex.getVertex()[0]);
            }
        }
    }
    // if we did not find any cycles, return an empty list
...
public static void contractSuperNodes(Hashtable<Integer, PriorityQueue<VertexWeighted>> allValuesClone, List<Integer> cycleNodes, int lastVertex , int counter) {
    ...
    public static void edmondAlgorithm(Hashtable<Integer, PriorityQueue<VertexWeighted>> allValues, int lastVertex , int counter) {
        ...
        List<Integer> cycleNodes = findAllZeroCycles(allValuesClone, lastVertex);
