...
public class closestPair {
    ...
    public static double findClosestPair(ArrayList<Coordinate> xCoordinates, ArrayList<Coordinate> yCoordinates) {
        ...
        return Math.min(delta, stripClosest(strip, delta, 0));
    }

    // Function to find the distance between the closest points of strip of a given
    // size
    private static double stripClosest(ArrayList<Coordinate> strip, double delta, int resume) {
        double min = delta;
        for (int i = resume; i < strip.size(); ++i) {
            for (int j = i + 1; j < i + 7 && j < strip.size() && (strip.get(j).y - strip.get(i).y) < min; ++j) {
                ...
                if (distance < min) {
                    ...
                    // update the stip again with the new min distance and the points
                    // if(!(strip.size() < 40)){
                        // }
                    stripClosest(strip, min, i);
                } 
                // else {
                //     break;
                // }
        ...
        try (Scanner sc = new Scanner(myFile)) {
            while (sc.hasNextLine()) {
                ...
                if (line.startsWith("*")) {
                    ...
                    System.out.println("\tClosest pair: " + String.format("(%10.5f, %10.5f)", closestPairPoint1.x, closestPairPoint1.y) + " - " + String.format("(%10.5f, %10.5f)", closestPairPoint2.x, closestPairPoint2.y));

                    // System.out.println("\tClosest pair: " + String.format("%10.5d",closestPairPoint1) + "-" + String.format("%10.5d",closestPairPoint2));
                    System.out.println(
                            "\tdistance = " + String.format("%11.6f", closestDistance) + " (" + (endTime - startTime)
                                    + " ms)");
